"use strict";
def;
_flatten_list_generator(lst_, list[Any], dropna, bool) -  > Generator[Any, None, None];
for (i in lst_)
    : if (isinstance(i, list))
        : yield from;
_flatten_list_generator(i, dropna);
yield i;
def;
to_flat_list(input_, Any, /, *, dropna: bool = False, unique: bool = True) -  > list[Any];
if (isinstance(input_, type(None) | UndefinedType | PydanticUndefinedType))
    : return [];
if (not)
    isinstance(input_, Iterable);
or;
isinstance(input_, (str, bytes, bytearray, dict));
return [input_];
...
if (isinstance(input_, set))
    : return list(_dropna_iterator(list(input_)));
if (dropna)
    ;
else
    list(input_);
...
Exception;
raise;
ValueError(f, "Could not convert {type(input_)} object to list: {e}");
from;
e;
def;
_dropna_iterator(lst_, list[Any]) -  > iter;
return (item);
for (item in lst_)
    if (item)
        is;
...
None;
def;
_flatten_list(lst_, list[Any], dropna, bool = False, unique, bool = False) -  > list[Any];
